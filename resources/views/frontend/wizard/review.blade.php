...
@php
  ...
  <div class="review-card">
    ...
    <div class="review-card">
      ...
      <div class="review-card-body">
        ...
        <div style="margin-top:1rem;display:flex;gap:.5rem;justify-content:flex-end;">
          <a href="{{ route('wizard.show', [$application, 'step' => 4]) }}" class="btn" style="background:#4305f0;border:1px solid #ddd;padding:.45rem .8rem;border-radius:6px;color:#ffffff;">
            Edit
          </a>
  ...
  <div class="review-card">
    ...
    <div class="review-card-body">
      ...
      <div style="margin-top:1rem;display:flex;gap:.5rem;justify-content:flex-end;">
        <a href="{{ route('wizard.show', [$application, 'step' => 5]) }}" class="btn" style="background:#1803d4;border:1px solid #ddd;padding:.45rem .8rem;border-radius:6px;color:#ffffff;">
          Edit
        </a>
            ...
            @if($existingDocs->count() > 0)
                    ...
                    @if($isImage && $url)
                      <div class="photo-item">
                        <img src="{{ $url }}" alt="{{ $doc->original_name }}"
                             data-src="{{ $url }}" data-name="{{ $doc->original_name }}" width="150px" height="80px"
              ...
              @else
                @php
                  ...
                  $isImage = in_array($ext, ['jpg','jpeg','png','webp','gif']);
                  $url = $first && asset('storage/'.$first->path) ? asset('storage/'.$first->path) : null;

                @endphp

                @if($isImage && $url)
                  <a href="javascript:void(0)" onclick="openPreviewModal(this)" data-src="{{ $url }}" data-name="{{ $first->original_name }}">
                    <img src="{{ $url }}" alt="{{ $first->original_name }}" class="preview-thumb" width="150px" height="80px">
                  </a>
                @elseif($url)
          ...
          <td class="actions">
            @if($existingDocs->count() > 0)
              ...
              <div class="d-flex flex-wrap align-items-center gap-2">
                @if($isPropertyPhotos)
                  <a href="{{ route('wizard.show', [$application, 'step' => 6]) }}"
                     class="btn btn-sm btn-warning d-flex align-items-center">
                    <i class="bi bi-pencil-square me-1"></i> Edit
                  </a>
                @else
                  @php
                    $fileUrl = ($first && asset('storage/'.$first->path)) ? asset('storage/'.$first->path) : null;
                  @endphp
                  ...
                  @if($fileUrl)
                    <a class="btn btn-sm d-flex align-items-center"
                       style="
                         ...
                         cursor:pointer;
                       "
                       href="{{ $fileUrl }}" download target="_blank" rel="noopener">
    ...
    <div class="card-body">
      <form method="POST" action="{{ route('wizard.submit', $application) }}" class="mt-4">
        ...
        <div class="form-check mb-3">
          <input class="form-check-input" type="checkbox" id="decl" >
