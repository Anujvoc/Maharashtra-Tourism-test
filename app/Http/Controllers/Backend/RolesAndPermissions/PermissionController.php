...
class PermissionController extends Controller implements HasMiddleware
{
    ...
    public function store(Request $request)
    {
        try {
            ...
            $data = $request->validate([
                'name' => ['required', 'string', 'max:255', 'unique:permissions,name', 'regex:/^[a-zA-Z0-9\s]+$/'],
                'guard_name' => ['nullable', 'string'],
                'group_name' => ['required', 'string', 'regex:/^[a-zA-Z0-9\s]+$/'],
            ]);
    ...
    public function update(Request $request, Permission $permission)
    {
        try {
            ...
            $data = $request->validate([
                'name' => ['required', 'string', 'max:255', 'unique:permissions,name,' . $permission->id, 'regex:/^[a-zA-Z0-9\s]+$/'],
                'group_name' => ['required', 'string', 'max:255', 'regex:/^[a-zA-Z0-9\s]+$/'],
            ]);
