...
$(function () {
    ...
    $('#previewSubmitBtn').on('click', function () {
        ...
        html += '<tr class="table-primary"><th colspan="2">General Details</th></tr>';

        html += `<tr><th>Applicant Name</th><td>${$('input[name="applicant_name"]').val() || '--'}</td></tr>`;
        html += `<tr><th>Provisional Certificate Number</th><td>${$('input[name="provisional_number"]').val() || '--'}</td></tr>`;
        html += `<tr><th>GST Number</th><td>${$('input[name="gst_number"]').val() || '--'}</td></tr>`;

        html += '<tr class="table-primary"><th colspan="2">Project Description</th></tr>';
        html += `<tr><th>Description</th><td>${$('textarea[name="project_description"]').val() || '--'}</td></tr>`;

        html += '<tr class="table-primary"><th colspan="2">Declaration</th></tr>';
        html += `<tr><th>Place</th><td>${$('input[name="declaration_place"]').val() || '--'}</td></tr>`;
        html += `<tr><th>Date</th><td>${$('input[name="declaration_date"]').val() || '--'}</td></tr>`;
        ...
        if (!$('#previewConfirmCheck').length) {
            $('#formPreviewBody').append(`
