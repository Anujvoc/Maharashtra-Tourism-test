...
RateLimiter::for('check-empid', function ($request) {
    return Limit::perMinute(1)->by($request->ip())
        ->response(function ($request, array $headers) {
            $retryAfter = $headers['Retry-After'] ?? 0;
            $retryTime = now()->addSeconds($retryAfter)->diffForHumans();

            return response()->json(['message' => ' Too many requests. Please try again after ' . $retryTime], 429);
        });
});
...
Route::middleware('guest')->group(function () {
    ...
    Route::post('reset-password', [NewPasswordController::class, 'store'])
        ->name('password.store')->middleware('throttle:3,1');
});
