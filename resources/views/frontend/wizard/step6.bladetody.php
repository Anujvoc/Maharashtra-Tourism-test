...
<section class="section">
  ...
  @php $application->loadMissing('documents'); @endphp
                ...
                @else
                  ...
                  @if($isImage)
                    <img src="{{ $url }}"
                         ...
                         onclick="openImageModal(this)">
                  @else
                    <div class="pdf-tile d-flex align-items-center" data-url="{{ $url }}" data-name="{{ $doc->original_name }}">
      ...
      <div class="modal-dialog modal-lg modal-dialog-centered">
        <div class="modal-content">
          ...
          <div class="modal-body text-center">
            <img src="" class="modal-preview-image" alt="Document Preview" id="modalImage">
...
$(document).ready(function() {
  ...
  // Show chosen filename on the label
  ...
  $(document).on('change', '.doc-file', function(){
    ...
    // For property photos, handle multiple files
      ...
      .done(res => {
        ...
        if (isPropertyPhotos) {
          ...
          if (res.is_image) {
            ...
            previewHtml = `
              <div class="pdf-tile d-flex align-items-center" data-url="${res.url}" data-name="${res.file_name}">
                <i class="bi bi-file-earmark-pdf text-danger me-2 fs-4"></i>
                <div>
        ...
        if (isPropertyPhotos && index === filesToUpload.length - 1) {
          fileInput.val('');
          fileInput.closest('.custom-file-input').find('.label-text').text('Choose Photos (Max 5)');
        } else if (!isPropertyPhotos) {
          fileInput.val('');
          fileInput.closest('.custom-file-input').find('.label-text').text('Choose File');
        }
      })
      .fail(function(xhr){
        ...
        fileInput.val('');
        fileInput.closest('.custom-file-input').find('.label-text').text(
          isPropertyPhotos ? 'Choose Photos (Max 5)' : 'Choose File'
        );
      });
    });
  });
  ...
  $(document).on('click', 'button.del', function(e){
    ...
    .done(() => {
      ...
      row.find('.preview-container').html('<div class="text-muted preview-placeholder"><i class="bi bi-image me-1"></i> No file uploaded</div>');
  ...
  $(document).on('click', '.del-photo', function(e){
    ...
    .done(() => {
      ...
      if (currentCount === 0) {
        $(`.preview-${cat}`).html('<div class="text-muted preview-placeholder"><i class="bi bi-image me-1"></i> No file uploaded</div>');
      }
    })
